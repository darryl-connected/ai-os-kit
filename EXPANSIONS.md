...
The kit ships lean on purpose. Six skills, ten folders, two framework references. That's it. As you use it, you'll outgrow the base — this guide tells you what to add, when, and why.
...
| `skills/` (or `.pi/skills/`, `.claude/skills/`) | Your skills. Add more via `/level-up`. |
| `aios-intake.md` | Source-of-truth for `/onboard`. Edit and re-run any time. |
| `APPEND_SYSTEM.md` (pi global) or `CLAUDE.md` (vault) | Root operating manual. Filled by `/onboard`. Edit when role/voice changes. |
| `AGENTS.md` (pi vault) | Vault-operations layer: edit policy, naming, frontmatter rules. |
| `inbox/` | Raw meeting captures. `/meeting-intake` files them. |
...
| `.claude/agents/` (Claude Code) or `.pi/extensions/` (pi) | You need a sub-assistant for repeatable, multi-step research/writing | Agents run on cheaper models in their own context — keep your main session lean |
...
- `APPEND_SYSTEM.md` (pi) or `CLAUDE.md` (Claude) — quarterly review; rewrite the persona/priorities section after `/level-up` Q90
...
- **Don't fork your operating manual.** One root identity file (pi: global `APPEND_SYSTEM.md`; Claude Code: vault `CLAUDE.md`). Sub-OS folders can have their own scoped CLAUDE.md, but the root is canonical.
