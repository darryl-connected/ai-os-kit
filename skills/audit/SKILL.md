...
Runs the **Four Cs Audit** on the current project. Reads (never writes) the project's identity layer, memory, skills, agents, MCPs, decisions, and references. Scores each of the Four Cs out of 25. Surfaces strengths and the top 3 leverage-weighted gaps with concrete next-step commands.
...
The audit looks for **patterns and intent**, not exact paths. File names and harness conventions vary. Use Glob and Read to check:

**Identity layer (persona/voice/priorities):**
- **pi:** `APPEND_SYSTEM.md` at the global pi agent directory (typically `~/.pi/agent/APPEND_SYSTEM.md`) is canonical. `SYSTEM.md` at the same location accepted as a full replacement.
- **Claude Code:** `CLAUDE.md` at the vault root.
- **Cursor/Cline:** `.cursorrules` or `CLAUDE.md` at the vault root.

**Project rules (vault management):**
- **pi:** vault-root `AGENTS.md` — edit policy, naming conventions, archive rules. Exists as a separate file by design (distinct from the identity layer).
- **Claude Code:** typically embedded in `CLAUDE.md` under a "Vault Operations" or similar section.

**Memory:** `MEMORY.md` (root), or `memory/` folder. Claude Code users may also have `~/.claude/projects/<id>/memory/MEMORY.md`.

**Skills:** Any of — `.pi/skills/*/SKILL.md`, `~/.pi/agent/skills/*/SKILL.md`, `.claude/skills/*/SKILL.md`, `.agents/skills/*/SKILL.md`. Count + frontmatter.

**Agents / Extensions:** Claude Code uses `.claude/agents/*.md`. Pi uses TS extensions in `.pi/extensions/*.ts` or `~/.pi/agent/extensions/*.ts`.

**Connection mechanisms** (any of these = "reachable"):
- MCPs: Claude Code — `.mcp.json`, `.claude/settings.json` (mcpServers key). Pi — `.pi/settings.json` (mcpServers key).
- API scripts: `scripts/*.py|.js|.ts` documented in identity layer or vault-root operations file
...
**Templates:** `templates/` (anywhere).
**Recurring triggers / cadence:** Skill names matching `morning-*`, `weekly-*`, `daily-*`, `monthly-*`, `standup`. Claude Code may also have `.claude/settings.json` hooks. Pi users rely on external schedulers (cron, GitHub Action) — infer from `README.md` or `.pi/settings.json`.
...
|---|---|---|
| Identity layer exists and is substantive (>200 words) | 5 | Read identity file (`APPEND_SYSTEM.md` or `CLAUDE.md` top section), count words |
| Identity / role / voice captured | 5 | Identity file mentions who the user is + role/mission, OR `references/voice.md` / `context/about-me.md` exists with role/voice content, OR `.claude/rules/*.md` is present |
...
| Criterion | Points | How to detect |
|---|---|---|
| 3+ skills installed | 10 | Count `SKILL.md` files across `.pi/skills/`, `~/.pi/agent/skills/`, `.agents/skills/`, `.claude/skills/` |
| 1+ user-built skill | 10 | Skill names not in the canonical set: `onboard`, `audit`, `level-up`, `skill-creator`, `skill-builder`, `decision`, `connect`, `connect-check`, `memory-prune`, `scaffold-skill`, `scaffold-agent`, `draft`, `standup` |
| 1+ agent defined | 5 | Claude Code: count `.claude/agents/*.md` ≥ 1. Pi: count `.pi/extensions/*.ts` or `~/.pi/agent/extensions/*.ts` ≥ 1 |

#### Cadence (25 pts)

| Criterion | Points | How to detect |
|---|---|---|
| 1+ recurring/scheduled trigger | 10 | Skill name matches `morning-*` / `daily-*` / `weekly-*` / `monthly-*` / `standup`. Claude Code may also have `.claude/settings.json` hooks. Pi may have an external scheduler documented in `README.md` or `.pi/settings.json`. |
| Recent activity / usage signal | 10 | Any `SKILL.md` modified within 30 days (across all skill dirs), OR `decisions/log.md` has entry within 30 days |
...
- Identity layer missing or thin: **3x** (foundation)
...
- **Need a new skill?** Recommend `skill-creator` (Anthropic) or `skill-builder` (if local), or "write `SKILL.md` at the appropriate harness skills directory (`.pi/skills/<name>/SKILL.md` for pi, `.claude/skills/<name>/SKILL.md` for Claude Code) with YAML frontmatter."
...
- **Need a recurring trigger?** "For pi: add a cron job or GitHub Action, or write a `.pi/extensions/` listener. For Claude Code: add a hook to `.claude/settings.json`. Or write a skill named `daily-*` you run each morning."
...
- **Read-only by default.** Never modify the identity layer file, vault-root operations file, memory, skills, or any project files. Only optional write is the audit report.
- **Be flexible about file names and harness conventions.** Don't penalize for using non-canonical names if intent is captured.
