...
6. **Identity layer file** — fill all `{{...}}` placeholders with the user's identity, stated priority, voice register summary, and a brief connections summary.
   - **pi users:** copy `templates/IDENTITY.md` content to the global `APPEND_SYSTEM.md` location (typically `~/.pi/agent/APPEND_SYSTEM.md`).
   - **Claude Code users:** the kit's `CLAUDE.md` already contains the merged identity + operations layer — edit it directly with the filled values.

> **Note:** Do NOT overwrite a vault-root `AGENTS.md` if one exists — it holds project-scoped vault rules (edit policy, naming conventions, archive rules). It's a separate concern from the system/identity layer and should be edited manually by the vault owner. (For Claude Code users, the kit's `CLAUDE.md` already includes the operations content — don't overwrite the kit's own `CLAUDE.md`; the user customizes it once and re-runs the install script if they reinstall.)
...
6. **No extra skills generated.** Don't scaffold `/today`, `/draft`, `/connect`, etc. The kit ships 6 skills; the user authors more via `/level-up`.
...
9. **Vault `AGENTS.md` / `CLAUDE.md` is read-only.** It belongs to the project owner. Don't scaffold it, don't touch it.
...
- Idempotency: re-run `/onboard` with one Q3 priority changed. Expected: only `context/priorities.md` and the identity layer file's priority section update; backup created in `archives/intake-{ts}/`.
